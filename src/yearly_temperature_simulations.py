...
beta = 5 # range of temperature going up and down
...
def yearly_mu(x, alpha, beta, p, shift, daysize = 0, daylength = 1/(2*np.pi)):
    return alpha + beta*np.sin(x/p + shift/p) + daysize*np.sin(x/daylength)
...
plt.plot(xhour,yearly_mu(xhour, alpha, beta, p, shift, daysize = daysize))
plt.fill_between(xhour, 
                 yearly_mu(xhour, alpha, beta, p, shift, daysize = daysize) - yearly_sigma(xhour, var, delta, p, ave_loc),
                 yearly_mu(xhour, alpha, beta, p, shift, daysize = daysize) + yearly_sigma(xhour, var, delta, p, ave_loc),
                 alpha = 0.5)
...
norms_day = [gen_norm_pdf(eT, yearly_mu(xhour[i], alpha, beta, p, shift, daysize = daysize),
                      yearly_sigma(xhour[i], var, delta, p, ave_loc), 2) for i in range(365*24)]
